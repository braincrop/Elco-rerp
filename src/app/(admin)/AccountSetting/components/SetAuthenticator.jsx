...
// import { Card, CardBody, CardHeader, Form, FormGroup, Label, Input, Button, Alert, 
//   Badge, Row, Col } from 'reactstrap';
...
import {
  Card, CardBody, CardHeader, Badge, Alert,
  Button, Row, Col, Form, FormGroup, Label, Input
} from 'reactstrap'
import { Icon } from '@iconify/react'

const SetAuthenticator = () => {
  const [selectedMethod, setSelectedMethod] = useState(null) // 'email' | 'authenticator'
  const [step, setStep] = useState(1)
  const [code, setCode] = useState('')
  const [enabled, setEnabled] = useState(false)
  ...
  // Dummy secret & email for QR
  const secretKey = 'JBSWYDPEHPK3PXP'
  const otpAuthUrl = `otpauth://totp/MyApp?secret=${secretKey}&issuer=MyApp`
  ...
  const handleSelectMethod = async (method) => {
    ...
    try {
      setLoading(true)
      // await api.post('/2fa/select-method', { method })
      console.log('API call: select method →', method)
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  const handleSendEmailCode = async () => {
    try {
      setLoading(true)
      // const res = await api.post('/2fa/send-email-otp')
  ...
  const handleVerifyAuthenticator = async (e) => {
    ...
    try {
      setLoading(true)
      // const res = await api.post('/2fa/verify-authenticator', { code })
        ...
        <Card className="border-0 rounded-3 mb-4 shadow-lg">
          <CardBody className="p-4">
            <div className="d-flex align-items-center gap-3 mb-4">
              ...
              <Badge color="success" className="ms-auto" pill>Active</Badge>
            </div>
            ...
            {/* Backup codes (authenticator only) */}
            {selectedMethod === 'authenticator' && (
              <div className="bg-light rounded-3 p-3 mb-4">
                <p className="fw-medium mb-2" style={{ fontSize: 13 }}>Backup Codes</p>
                <p className="text-muted mb-3" style={{ fontSize: 12 }}>
                  Save these codes somewhere safe. Each can be used once if you lose access to your authenticator.
                </p>
                <Row className="g-2">
              ...
              <CardBody className="p-4">
                <div className="d-flex align-items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="text-muted" style={{ fontSize: 12 }}>Verify via email code</div>
                  </div>
                </div>
              ...
              <CardBody className="p-4">
                <div className="d-flex align-items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="text-muted" style={{ fontSize: 12 }}>Verify via QR code app</div>
                  </div>
                </div>
                <p className="text-muted mb-0" style={{ fontSize: 13 }}>
          ...
          <CardBody className="p-4">
            {!emailSent ? (
              <div>
                ...
                <Button color="primary" className="rounded-2 px-4" onClick={handleSendEmailCode} disabled={loading}>
                  <Icon icon="mdi:email-send" width={16} className="me-1" />
              ...
              <Form onSubmit={handleVerifyEmail}>
                ...
                <FormGroup className="mb-4">
                  ...
                  <Input
                    ...
                    placeholder="000000"
                  />
                  <small className="text-muted">Didn't receive?{' '}
                    <span role="button" className="text-primary" onClick={handleSendEmailCode}>Resend</span>
                  </small>
                </FormGroup>
                <div className="d-flex gap-2">
                  <Button type="submit" color="success" className="rounded-2 px-4"
  ...
  // ════════════════════════════════════════════════════════════════
  // AUTHENTICATOR APP FLOW (original steps with real QR)
  // ════════════════════════════════════════════════════════════════
  return (
    <div>
      <div className="d-flex align-items-center gap-2 mb-4">
        ...
        <CardBody className="p-4">
            ...
            <div>
              ...
              <Row className="g-3 mb-4">
                {[
                  ...
                  <Col xs={12} key={i}>
                    <div className="d-flex align-items-center gap-3 border rounded-3 p-3">
                      <div className="bg-primary bg-opacity-10 text-primary rounded-2 d-flex align-items-center justify-content-center"
                        style={{ width: 40, height: 40 }}>
                        <Icon icon={app.icon} width={22} />
                      </div>
                      <div>
                        <div className="fw-medium custom-text" style={{ fontSize: 14 }}>{app.name}</div>
                        <div className="text-muted custom-text" style={{ fontSize: 12 }}>{app.platforms}</div>
                      </div>
                    </div>
                  </Col>
                ))}
          ...
          {step === 2 && (
            <div>
              ...
              <div className="text-center mb-4">
                ...
                <div className="text-muted mb-2" style={{ fontSize: 12 }}>Or enter this key manually:</div>
                <div className="bg-light border rounded-2 px-4 py-2 d-inline-block fw-semibold"
                  style={{ fontFamily: 'monospace', letterSpacing: '0.1em', fontSize: 15 }}>
                  {secretKey}
                </div>
              </div>
              <div className="d-flex gap-2">
                ...
                <Button color="outline-secondary" className="rounded-2" onClick={() => setStep(1)}>Back</Button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <Form onSubmit={handleVerifyAuthenticator}>
              ...
              <FormGroup className="mb-4">
                <Label className="text-muted fw-medium" style={{ fontSize: 12 }}>VERIFICATION CODE</Label>
                <Input
                  type="text"
                  maxLength={6}
                  value={code}
              ...
              <div className="d-flex gap-2">
                ...
                <Button color="outline-secondary" className="rounded-2" onClick={() => setStep(2)}>Back</Button>
              </div>
            </Form>
          )}
